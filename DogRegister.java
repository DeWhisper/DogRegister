...
public class DogRegister {
    ...
    private void removeOwner(){
        if (ownerRegisterIsEmpty()) {
            //errors have been printed and return to menu
            return;
        }
        String ownerName = input.readLine("Enter owner name");
        while(ownerNameIsBlank(ownerName)){
            ownerName = input.readLine("Enter owner name");
        }
    ...
    private void removeDogFromOwner(){
        if(dogRegisterIsEmpty() || ownerRegisterIsEmpty()){
            return;
        }

        String dogName = input.readLine("Enter dog name");

        if(!dogCollection.containsDog(dogName)){    //If Dog is not in register
            System.out.println("Error: The dog " + dogName + " doesn't exist.\n");
            return;
        }
        if(dogCollection.getDog(dogName).getOwner() == null){   //If dog has no owner
            System.out.println("Error: The dog " + dogName + " doesn't have no owner.\n");
            return;
        }
        ...
        owner.removeDog(dogToRemove);
        dogToRemove.setOwner(null);
        System.out.println("The dog " + dogName + " now have no owner.\n");
        return;
    }
    /*
